...
def get_tickers():
    try:
        ...
        raw400 = [str(t).replace('.','-').strip() for t in df1[c1].dropna().tolist()]
        raw600 = [str(t).replace('.','-').strip() for t in df2[c2].dropna().tolist()]
        # FILTER: Alpaca doesn't like $, -, / etc. Keep only A-Z0-9
        def clean(lst):
            cleaned = []
            for t in lst:
                t = t.strip().upper()
                if not t or '$' in t or '/' in t:
                    continue
                # CWEN-A -> skip, keep only plain symbols for now
                if '-' in t and len(t) > 5:
                    continue
                if not re.match(r'^[A-Z]{1,5}$', t.replace('-','')):
                    # allow 1-5 letters, skip weird ones
                    if '-' in t:
                        continue
                cleaned.append(t)
            return cleaned
...
st.caption(f"Universe {len(t400)} + {len(t600)} (filtered for Alpaca)")
...
def calc_alpaca(tickers, top_n, label, sd, ed):
    ...
    for batch_idx in range(0, len(tickers), 50):
        ...
        # Retry loop for bad symbols in batch
        remaining = batch[:]
        while remaining:
            try:
                ...
                msg = str(e)
                # Extract invalid symbol from Alpaca error like "invalid symbol: CWEN-A"
                m = re.search(r'invalid symbol[: ]+([A-Z0-9\.\-\$]+)', msg, re.I)
                if m:
                    bad_sym = m.group(1).strip().upper()
                    invalid.append(f"{bad_sym} - invalid for Alpaca")
                    remaining = [x for x in remaining if x!= bad_sym]
                    if not remaining:
                        break
                    warn_box.write(f"Skipping {bad_sym}, retrying batch...")
                    continue
                else:
                    warn_box.warning(f"Batch error, splitting: {msg[:120]}")
                    # split batch in half if unknown error
                    if len(remaining) == 1:
                        invalid.append(f"{remaining[0]} - {msg[:60]}")
                        break
                    mid = len(remaining)//2
                    # process halves recursively via remaining loop
...
if st.button("Calculate Momentum 100 - Alpaca", type="primary", disabled=st.session_state.running):
    if not API_KEY or not SECRET_KEY:
        error_box.error("Add ALPACA_API_KEY and ALPACA_SECRET_KEY in Secrets")
    else:
        ...
        try:
            ...
            combined = pd.concat([df400][df600]) if not df400.empty or not df600.empty else pd.DataFrame()
            if combined.empty:
                error_box.error(f"No data - got {len(bad400+bad600)} invalid. Check keys. Last invalid: {(bad400+bad600)[:3]}")
            else:
...
if st.session_state.result:
    ...
    with st.expander(f"Invalid tickers skipped for Alpaca: {len(bad)} - tap to check"):
