    ...
    try:
        ...
        def clean(lst):
            ...
            for t in lst:
                ...
                if "$" in t or "/" in t or not re.match(r"^[A-Z]{1,5}$", t):
                    continue
                out.append(t)
...
# --- NEW UI LAYOUT: Portfolio ABOVE Button ---
...
# --- Button right under portfolio select ---
error_box = st.empty()
warn_box = st.empty()
log_box = st.empty()
    ...
    for batch_idx in range(0, len(tickers), 50):
        ...
        while remaining:
            try:
                ...
                if bars.empty:
                    invalid.extend([f"{tk} - no bars" for tk in remaining]); break
                for tk in remaining:
                    if tk not in bars.index.get_level_values(0):
                        invalid.append(f"{tk} - not returned"); continue
                    hist=bars.loc[tk]
                    if len(hist)<180:
                        invalid.append(f"{tk} - <180d"); continue
                ...
                if m:
                    ...
                    if len(remaining)==1:
                        invalid.append(f"{remaining[0]} - {msg[:80]}"); break
                    mid=len(remaining)//2
...
if st.button("Calculate Momentum 100 - Alpaca", type="primary", disabled=st.session_state.running, use_container_width=True):
    if not API_KEY or not SECRET_KEY:
        ...
        st.session_state.result=None
        error_box.empty(); warn_box.empty()
        try:
